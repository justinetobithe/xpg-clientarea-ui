    ...
    return (
        <>
            {parts.map((p, i) => {
                ...
                re.lastIndex = 0;
                return isHit ? (
                    <span key={i} className="rounded-md bg-primary/20 px-1.5 py-0.5 text-primary font-semibold">
                        {p}
                    </span>
                ) : (
                    <span key={i}>{p}</span>
        ...
        <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                    ...
                    <div className="text-white/60 text-sm mt-1 break-words">{desc}</div>
                </div>
            </div>
        </div>
    );
            ...
            <div className="p-5">
                <div className="flex gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="text-white font-semibold text-lg leading-snug break-words">
                        ...
                        <div className="text-white/60 text-sm mt-1">
                            {hasNested ? (
                                <span className="inline-flex items-center gap-2">
                                    ...
                                    <span className="break-words">{t("search.gameCard.matchingFound")}</span>
                                </span>
                            ) : (
                                <span className="inline-flex items-center gap-2">
                                    <FileText className="h-4 w-4 text-white/50 shrink-0" />
                                    <span className="break-words">{t("search.gameCard.noMatching")}</span>
                                </span>
                            )}
                        </div>
                    </div>
                ...
                <div className="mt-4 flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-end">
                    ...
                    {sections.length > 0 ? (
                        ...
                        >
                            ...
                            <span className="ml-2 text-sm font-semibold">{open ? t("search.actions.collapse") : t("search.actions.expand")}</span>
                        </button>
                    ) : null}
                </div>
            </div>
        ...
        <PageShell crumb={t("search.crumb")} title={t("search.title")} subtitle={t("search.subtitle")} right={headerRight}>
            <div className="pb-[env(safe-area-inset-bottom)]">
                <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.05] via-black/20 to-black/30 p-5 md:p-6 shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
                    <div className="flex items-start justify-between gap-4 flex-col md:flex-row">
                        <div className="flex items-start gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                <div className="text-white font-semibold text-xl break-words">
                                    {q ? (
                                        <>
                                            {t("search.hero.resultsFor")} <span className="text-primary break-words">{q}</span>
                                        </>
                                    ) : (
                                        t("search.hero.startSearching")
                                    )}
                                </div>
                                <div className="text-white/60 text-sm mt-1 break-words">
                                ...
                                <div className="mt-3 md:hidden inline-flex items-center gap-2 rounded-xl border border-white/10 bg-black/25 px-3 py-2">
                                    ...
                                    <span className="text-sm text-white/70 break-words">
                                        ...
                                        <span className="text-white font-semibold break-words">{q || t("search.common.dash")}</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                {q && !isLoading && !error && totalCount === 0 ? (
                    <div className="mt-6">
                        <EmptyCard icon={SearchIcon} title={t("search.emptyStates.noResults.title")} desc={t("search.emptyStates.noResults.desc", { q })} />
                    </div>
                ) : null}
                    ...
                    <div className="mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
                        <div className="lg:col-span-4 space-y-4">
                            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
                                <div className="flex items-center justify-between mb-3 gap-3">
                                    ...
                                    <Link to="/announcements" className="text-primary text-sm font-semibold hover:opacity-80 shrink-0">
                        ...
                        <div className="lg:col-span-8 space-y-4">
                            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
                                ...
                                <div className="text-white/60 text-sm mt-1 break-words">{t("search.panels.games.hint")}</div>
                            </div>
